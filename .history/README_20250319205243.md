...
## Lab Overview
In this lab, participants install and configure Grafana on an Ubuntu VM, then connect it to Azure Monitor using a managed identity. They learn how to gather and visualize real-time metrics—such as CPU usage, memory utilization, and network traffic—by creating a custom dashboard. The end result is a fully functioning performance monitoring setup for their Ubuntu server.

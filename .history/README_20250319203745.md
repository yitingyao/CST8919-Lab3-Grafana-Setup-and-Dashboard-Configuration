...
CPU usage is the proportion of the VM’s processing power that is actively in use. memory usage reflects how much RAM is currently occupied by running processes, network in measures how much data is received by the VM (inbound traffic), and network out measures how much data is sent from the VM (outbound traffic).
